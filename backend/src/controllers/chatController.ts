...
import { buildGmailQueryFromMessage, isGmailQuery, wantsFreshEmails } from "../utils/query";
...
export async function chatHandler(req: Request, res: Response) {
  ...
  try {
    ...
    if (isGmailQuery(message)) {
      ...
      const shouldForce = Boolean(forceRefreshEmails) || wantsFreshEmails(message);

      const { q } = buildGmailQueryFromMessage(message);
      if (q) {
        const qHash = encodeURIComponent(q);
        const queryCacheKey = `${cacheKey}:q:${qHash}`;
        const cachedQueryEmails = shouldForce ? null : await redisClient.get(queryCacheKey);
        if (cachedQueryEmails) {
          if (process.env.NODE_ENV !== "production") console.log("Using cached query emails from Redis", q);
          emailContext = JSON.parse(cachedQueryEmails) as Email[];
        } else {
          if (process.env.NODE_ENV !== "production") console.log("Fetching query-filtered emails from Gmail API", q);
          ...
          systemMetaNotes.push(`Search matched ~${total} messages (showing up to ${emails.length}).`);
        }
      } else {
        const cachedEmails = shouldForce ? null : await redisClient.get(cacheKey);
        if (cachedEmails) {
          if (process.env.NODE_ENV !== "production") console.log("Using cached emails from Redis");
          emailContext = JSON.parse(cachedEmails) as Email[];
        } else {
          if (process.env.NODE_ENV !== "production") console.log("Fetching fresh emails from Gmail API (recent)");
          const { emails, total } = await fetchGmailEmails(
            req.session.tokens.access_token!,
          ...
          emailContext = emails as Email[];
          systemMetaNotes.push(`Loaded recent emails (showing ${emails.length} of ~${total}).`);
        }
      }
